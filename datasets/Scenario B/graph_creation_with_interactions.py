# -*- coding: utf-8 -*-
"""Graph_creation_with_interactions.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CHXngvZ44CKKtVFxfsb2mFkdYrS1XEn_
"""
...
#same thing done on the one with madeup phenotype and interactions
...
print_graph_info(dataset_list[0][1][4]['Graph'])

plot_graph(dataset_list[0][0][3]['Graph'])

# cod to see wich genes were added dyrectly from string and wich are inside the extracted baìut that are not in the nodes for all the thresholds
...
# removes from all the graphs the node added by string
k = 0
...
print_graph_info(dataset_list[2][0][0]['Graph'])
#threshold = 0: edges = 1143
#threshold = 0.25: edges = 1143
#threshold = 0.50: edges = 798
#threshold = 0.75: edges = 302

#extract the nodes to add for all the thresholds removing those that starts with those prefix
nodes_to_add = []
...
# remove the genes that doesn't codify a protein for all the thresholds
...
# add all the remaining nodes to the graphs:
n = 0
...
print_graph_info(dataset_list[2][0][4]['Graph'])

plot_graph(dataset_list[2][0][4]['Graph'])

#remove from all the 4 datasets for ML the snps related to nodes not included
...
adj_matrix = nx.adjacency_matrix(dataset_list[1][0][0]['Graph']).todense()
...
with open('/content/drive/MyDrive/PROJECT/dataset_Age_Sex_madeup_pheno_interactions.pickle', 'wb') as handle:
    pickle.dump(new_dataset, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('/content/drive/MyDrive/PROJECT/dataset_NCBI_graph_madeup_pheno_interactions.pickle', 'wb') as handle:
